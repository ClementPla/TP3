...
def load_data(dataset):
    ...
    if (not os.path.isfile(dataset)) and data_file == 'mnist.pkl.gz':
        ...
        basedir = os.path.dirname(dataset)
        if not os.path.exists(basedir):
            os.makedirs(basedir)

        open(dataset, 'a').close()
...
def data_augmentation(input_set, target):
    print('... Augmenting data')
    augmented_data_set_size = int(len(input_set)*PERCENTAGE_OF_AUGMENTED_DATA)
    index_map = np.arange(augmented_data_set_size)
    ...
    def plot_images(set_images, set_DA):
        plt.subplot(241);
        plt.axis('off');
        plt.imshow(set_images[50], cmap='gray')
        plt.subplot(242);
        plt.axis('off');
        plt.imshow(set_DA[50].reshape(28, 28), cmap='gray')
        plt.subplot(243);
        plt.axis('off');
        plt.imshow(set_images[55], cmap='gray')
        plt.subplot(244);
        plt.axis('off');
        plt.imshow(set_DA[55].reshape(28, 28), cmap='gray')
        plt.subplot(245);
        plt.axis('off');
        plt.imshow(set_images[10], cmap='gray')
        plt.subplot(246);
        plt.axis('off');
        plt.imshow(set_DA[10].reshape(28, 28), cmap='gray')
        plt.subplot(247);
        plt.axis('off');
        plt.imshow(set_images[15], cmap='gray')
        plt.subplot(248);
        plt.axis('off');
        plt.imshow(set_DA[15].reshape(28, 28), cmap='gray')
        plt.show()
...
def construct_multilayer_model(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
   ...
   """
    ...
    # Define layers as a list of tuple. Each tuple indicates
    # the numbers of hidden neurons and the associated activation function
    # The list's size defines the number of hidden layers.
    fc_layers = [(500, T.tanh), (200, T.tanh)]
